...
def export_channel_metadata(ch_obj: ChannelWrapper):
    ...
    illumination_type: Optional[str] = None if lch_obj.getIllumination() is None else lch_obj.getIllumination().getValue()
    pinhole_size: Optional[float] = lch_obj.getPinHoleSize()
    acquisition_mode: Optional[str] = None if lch_obj.getMode() is None else lch_obj.getMode().getValue()
    contrast_method: Optional[str] = None if lch_obj.getContrastMethod() is None else lch_obj.getContrastMethod().getValue()
    excitation_wavelength: Optional[float] = None if lch_obj.getExcitationWave() is None else lch_obj.getExcitationWave().getValue()
    emission_wavelength: Optional[float] = None if lch_obj.getEmissionWave() is None else lch_obj.getEmissionWave().getValue()
    ...
    color: Optional[str] = None if ch_obj.getColor() is None else ch_obj.getColor().getHtml()
